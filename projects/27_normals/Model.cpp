...
Model::Model(const char* file, unsigned int instancing,
	...
	// Same ordering rule as the two above: traverseNode() reads it on the way
	// down, so it must be set before the walk starts.
...
void Model::traverseNode(unsigned int nextNode, glm::mat4 matrix) {
	...
	// Dropped before the transform is built, and takes its subtree with it: a
	// child's placement only means anything relative to a parent being drawn.
...
std::vector<Texture> Model::getTextures(const json& primitive) {
	...
	// A flat colour as a 1x1 texture. A material with no map is not broken: glTF
	// lets it carry only a baseColorFactor, defaulting to white. One pixel of that
	// keeps the pipeline texture-based with no branch and no extra uniform.
	...
	// Units follow the material ROLE, not a running count: Mesh::Draw binds one
	// diffuse and one specular per mesh, so two units is all a mesh can use.
	// This was loadedTex.size(), which climbed past the 16 units GL 3.3 guarantees
	// and overwrote the shadow map on unit 2 whenever the model was drawn.
	...
	if (primitive.find(GLTF_KEY_MATERIAL) != primitive.end())
	{
		...
		if (materialIndex < JSON[GLTF_KEY_MATERIALS].size())
		{
			...
			// Sketchfab and many FBX->glTF converters put the albedo map under
			// KHR_materials_pbrSpecularGlossiness instead of the core PBR block.
			// Without this every material reads as textureless.
	...
	// Resolution order. The old filename guess is gone on purpose: it turned "no
	// data" into confidently wrong data, painting one image across every mesh.
	//   1. declared texture that loads -> use it
	//   2. colour factor only -> 1x1 of that colour;  3. broken -> checker + warn
