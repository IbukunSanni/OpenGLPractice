...
void run()
{
	...
	// The regular pass shades the crow; the outline pass outputs solid white.
	...
	// Keep nearer fragments and record successfully drawn crow pixels.
	...
	// The second asset is a slightly expanded copy used only for the border.
	Model model(modelPath.c_str());
	Model outline(outlinePath.c_str());
	// Orbit around the loaded model's world-space bounds center.
	...
	while (!glfwWindowShouldClose(window.get()))
	{
		...
		// First pass: shade the crow and write 1 wherever it is visible.
		glStencilFunc(GL_ALWAYS, 1, 0xFF);
		glStencilMask(0xFF);

		model.Draw(shaderProgram, camera);

		// Second pass: draw the expanded mesh only outside the crow's pixels.
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
		glStencilMask(0x00);
		// Ignore the crow's depth while the stencil test hides its interior.
		glDisable(GL_DEPTH_TEST);

		// crow-outline already contains the enlarged geometry.
		outline.Draw(outliningProgram, camera);

		// Restore writable stencil and depth state for the next frame.
		glStencilMask(0xFF);
