...
int run()
{
	...
	GLFWwindow* window = glfwCreateWindow(width, height, "13_model_loading", nullptr, nullptr);
	if (window == NULL)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		titleStream << std::fixed << std::setprecision(2)
			<< "13_model_loading | Pos(" << camera.Position.x << ", " << camera.Position.y << ", " << camera.Position.z << ") "
