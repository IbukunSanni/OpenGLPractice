...
public:
	...
	// Draws the model at an arbitrary world transform. Unlike the T/R/S
	// overload above -- which is applied INSIDE the glTF node chain and so gets
	// reinterpreted by it -- this is applied after the node matrix, so it means
	// plain world units. That is what lets one loaded Model be drawn at many
	// places in a frame.
