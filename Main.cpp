...
int main()
{
	...
	//   Attribute 1 = color    (3 floats starting at offset 3 floats in)
	// The stride (6 * sizeof(float)) is the total size of one vertex's data.
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindTexture(GL_TEXTURE_2D, texture);
		// Bind our VAO so OpenGL knows which vertex data and configuration to use.
		VAO1.Bind();
